...
This folder contains the C++ backend that maxes out CPU, RAM, GPU, and VRAM when the corresponding options are enabled in the GUI. **GPU/VRAM stress** uses **CUDA** on NVIDIA GPUs and **OpenCL** on AMD/Intel GPUs (or when CUDA is not available); the backend picks the right API at runtime.
...
```bat
cd C:\Users\User\Desktop\All-in-one-Benchmark\stress_backend
mkdir build
cd build
...
## Build with OpenCL (AMD/Intel GPU stress)

OpenCL is used for GPU/VRAM stress when no NVIDIA GPU is present (or when the build has no CUDA). CMake looks for OpenCL automatically; most GPU drivers provide it. To support **both** NVIDIA (CUDA) and AMD/Intel (OpenCL) in one exe, build with CUDA and OpenCL both available.
...
## TO build make sure you have dependcies and run
python -m PyInstaller All-in-one-Benchmark.spec